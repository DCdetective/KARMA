...
class Model(torch.nn.Module):
    def __init__(self, configs):
        ...
        self.mamba1 = Mamba(d_model=self.d_model_param1, d_state=self.configs.d_state, d_conv=self.configs.d_conv,
                            expand=self.configs.expand)
        self.mamba2 = Mamba(d_model=self.configs.n2, d_state=self.configs.d_state, d_conv=self.configs.d_conv,
                            expand=self.configs.expand)
        self.mamba3 = Mamba(d_model=self.configs.n1, d_state=self.configs.d_state, d_conv=self.configs.d_conv,
                            expand=self.configs.expand)
        self.mamba4 = Mamba(d_model=self.d_model_param2, d_state=self.configs.d_state, d_conv=self.configs.d_conv,
                            expand=self.configs.expand)
